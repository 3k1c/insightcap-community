...
**文件狀態**：歷史驗證快照。InsightCAP 目前版本狀態為 `0.9.0-beta.1`，此報告不代表正式版認證。

---

## 1. 知識庫壓力與穩定性測試 (Knowledge Base Stress Test)

- **測試腳本**：歷史測試項目，原獨立腳本已不在目前 source tree。
...
為了繞過 Tauri 在測試環境下的 DLL 載入限制，專案保留以下獨立二進制 (Stand-alone Binaries) 測試腳本進行核心驗證：
...
**聲明**：以上腳本供開發者手動執行或於 CI 流程中使用。在 Beta App 中，相關邏輯由對應的核心 Service 承載；此文件只記錄 2026-04-27 當時的驗證結果。
