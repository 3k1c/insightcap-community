...
pub async fn import_kb(
    ...
    for i in 0..archive.len() {
        ...
        let out_path = kb_root.join(file.name());
    ...
    let pool = SqlitePoolOptions::new()
        ...
        .await
        .map_err(|_| "INVALID_MNEMONIC".to_string())?;  // key 不符 → mnemonic 錯誤
...
/// 刪除知識庫：清除所有 captures / sources / memory_chunks / 向量索引
#[tauri::command]
pub async fn delete_kb(
    ...
    // 2. 清空主要資料表
    let tables = [
        "captures", "sources", "memory_chunks",
        "conversations", "messages", "tags", "capture_tags",
    ];
    for table in &tables {
        sqlx::query(&format!("DELETE FROM {}", table))
            .execute(db)
            .await
            .map_err(|e| format!("清除 {} 失敗: {}", table, e))?;
    }
    ...
    println!("[DeleteKB] All knowledge data deleted.");
    Ok(())
}
