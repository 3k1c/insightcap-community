...
; Uninstaller Pages
; 1. InsightCAP data removal options page
Var DeleteAppDataCheckbox
Var DeleteAppDataCheckboxState
Var InsightCAPClearKeysCheckbox
Var InsightCAPDeleteCustomKbCheckbox
Var InsightCAPClearKeysState
Var InsightCAPDeleteCustomKbState
Var InsightCAPCustomKbPath

Function un.InsightCAPLoadCustomKbPath
  StrCpy $InsightCAPCustomKbPath ""

  InitPluginsDir
  FileOpen $2 "$PLUGINSDIR\insightcap-kb-path.ps1" w
  FileWrite $2 "$$bundle = '${BUNDLEID}'$\r$\n"
  FileWrite $2 "$$roots = @([Environment]::GetFolderPath('ApplicationData'), [Environment]::GetFolderPath('LocalApplicationData'))$\r$\n"
  FileWrite $2 "function Test-UnderPath($$path, $$root) {$\r$\n"
  FileWrite $2 "  try {$\r$\n"
  FileWrite $2 "    $$full = [IO.Path]::GetFullPath($$path).TrimEnd([IO.Path]::DirectorySeparatorChar, [IO.Path]::AltDirectorySeparatorChar)$\r$\n"
  FileWrite $2 "    $$base = [IO.Path]::GetFullPath($$root).TrimEnd([IO.Path]::DirectorySeparatorChar, [IO.Path]::AltDirectorySeparatorChar)$\r$\n"
  FileWrite $2 "    return $$full.Equals($$base, [StringComparison]::OrdinalIgnoreCase) -or $$full.StartsWith($$base + [IO.Path]::DirectorySeparatorChar, [StringComparison]::OrdinalIgnoreCase) -or $$full.StartsWith($$base + [IO.Path]::AltDirectorySeparatorChar, [StringComparison]::OrdinalIgnoreCase)$\r$\n"
  FileWrite $2 "  } catch { return $$false }$\r$\n"
  FileWrite $2 "}$\r$\n"
  FileWrite $2 "foreach ($$root in $$roots) {$\r$\n"
  FileWrite $2 "  $$appData = Join-Path $$root $$bundle$\r$\n"
  FileWrite $2 "  $$bootstrap = Join-Path $$appData 'bootstrap.json'$\r$\n"
  FileWrite $2 "  if (Test-Path -LiteralPath $$bootstrap) {$\r$\n"
  FileWrite $2 "    try {$\r$\n"
  FileWrite $2 "      $$kb = ((Get-Content -LiteralPath $$bootstrap -Raw) | ConvertFrom-Json).kb_path$\r$\n"
  FileWrite $2 "      $$db = Join-Path $$kb '.insightcap\insightcap.db'$\r$\n"
  FileWrite $2 "      if ($$kb -and -not (Test-UnderPath $$kb $$appData) -and (Test-Path -LiteralPath $$db)) { [Console]::Write($$kb); break }$\r$\n"
  FileWrite $2 "    } catch {}$\r$\n"
  FileWrite $2 "  }$\r$\n"
  FileWrite $2 "}$\r$\n"
  FileClose $2

  nsExec::ExecToStack '"$SYSDIR\WindowsPowerShell\v1.0\powershell.exe" -NoProfile -ExecutionPolicy Bypass -File "$PLUGINSDIR\insightcap-kb-path.ps1"'
  Pop $0
  Pop $1

  ${If} $0 = 0
    StrCpy $InsightCAPCustomKbPath "$1"
  ${EndIf}
FunctionEnd

Function un.InsightCAPUninstallOptionsPage
  ${If} $PassiveMode = 1
    Abort
  ${EndIf}
  ${If} $UpdateMode = 1
  ...
  ${Else}
    ${NSD_CreateLabel} 14u 120u 94% 24u "$InsightCAPCustomKbPath"
    Pop $1
  ${EndIf}
...
Function un.InsightCAPUninstallOptionsLeave
  ${NSD_GetState} $DeleteAppDataCheckbox $DeleteAppDataCheckboxState
  ${NSD_GetState} $InsightCAPClearKeysCheckbox $InsightCAPClearKeysState
  ${NSD_GetState} $InsightCAPDeleteCustomKbCheckbox $InsightCAPDeleteCustomKbState
