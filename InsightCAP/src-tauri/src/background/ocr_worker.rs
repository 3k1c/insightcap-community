                    ...
                    match crate::ocr::perform_ocr(&ocr_input).await {
                        Ok(raw_text) => {
                            ...
                            if let Ok(settings) = crate::settings::store::get_settings(&pool).await {
                                if let Some(vc) = crate::providers::llm::vision::VisionConfig::from_settings(
                                    &settings.ai_models.vision_model,
                                ) {
                                    if let Some(vision_text) = crate::providers::llm::vision::try_vision_enhance(
                                        &vc,
                                        &raw_bytes,
                            ...
                            let vector_id_opt: Option<i64> = match app_state.embedder.embed(&clean_text).await {
                                Ok(vec) => {
                                    let vid = {
                                    ...
                                    match app_state.vector_store.add_vector(vid, &vec).await {
                                        ...
                                        Err(e) => { eprintln!("[OCR-WORKER] vector store error: {}", e); None }
                                    }
                                }
                                Err(e) => { eprintln!("[OCR-WORKER] embed error: {}", e); None }
                            };
                            ...
                            tokio::spawn(async move {
                                ...
                                let _ = tag_engine.process_new_capture(&tag_cid, &tag_content).await;
                            });
                            ...
                            tokio::spawn(async move {
                                ...
                                let _ = rel_engine.analyze_and_link(&rel_cid, "capture", &rel_content).await;
                            });
