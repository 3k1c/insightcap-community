...
pub async fn ingest_file(
    ...
    let parsed = crate::capture::file_parser::parse_file(&kb_path, &file_path, vision_config.as_ref()).await?;
    let now = Utc::now().to_rfc3339();
    ...
    for f_chunk in parsed.chunks {
        ...
        for (idx, para) in paragraphs.into_iter().enumerate() {
            ...
            if let Err(e) = space_engine.assign_to_space(&chunk_id, &para).await {
                eprintln!("[IngestFile] Space assignment failed for {}: {}", &chunk_id[..8], e);
            }
