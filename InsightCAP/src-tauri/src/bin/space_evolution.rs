...
async fn print_spaces(pool: &sqlx::SqlitePool) -> Result<(), Box<dyn std::error::Error>> {
    let rows = sqlx::query("SELECT name, chunk_count FROM spaces WHERE is_archived = 0").fetch_all(pool).await?;
    println!("  [STATUS] Spaces ({}):", rows.len());
    if rows.is_empty() { println!("    (No active spaces)"); }
    for r in rows {
        println!("    - {}: {} items", r.get::<String, _>("name"), r.get::<i64, _>("chunk_count"));
    }
    Ok(())
}
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let state = AppState::new(pool.clone(), kb_path, vector_store, embedder, Arc::new(shutdown_tx));

    let engine = SpaceEngine::new(state.db.clone(), state.embedder.clone(), state.vector_store.clone());

    println!("\n--- Step CLEANUP: Removing old test data ---");
    sqlx::query("DELETE FROM captures WHERE id LIKE 'space-test-%'").execute(&pool).await?;
    sqlx::query("DELETE FROM sources WHERE id = 'space-test'").execute(&pool).await?;
    ...
    let broad_rust = "A comprehensive guide to Rust programming language, focusing on system safety and speed.";
    ...
    sqlx::query("DELETE FROM captures WHERE id LIKE 'space-test-w2-%'").execute(&pool).await?;
