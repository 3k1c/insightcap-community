...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let ids = engine.extract_reminders(conv_id, summary, dialogue, timestamp, None).await?;
    ...
    if ids.is_empty() {
        ...
        for id in &ids {
            ...
            println!("  - [{}] Title: {}, Date: {:?}, Time: {:?}, Status: {}, Pending: {}", 
                     id, r.get::<String, _>("title"), r.get::<Option<String>, _>("event_date"), 
                     r.get::<Option<String>, _>("event_time"), r.get::<String, _>("status"), 
    ...
    if let Some(reminder_id) = ids.get(0).cloned() {
        ...
        let pending: i32 = sqlx::query_scalar("SELECT pending_confirm FROM reminders WHERE id = ?").bind(&reminder_id).fetch_one(&pool).await?;
        ...
        engine.update_reminder_status(&reminder_id, "completed").await?;
        let status: String = sqlx::query_scalar("SELECT status FROM reminders WHERE id = ?").bind(&reminder_id).fetch_one(&pool).await?;
        println!("  Final Status: {}", status);
    }

    println!("\n--- Step 5: Handling Surprise Event (Update via Extraction) ---");
    // Simulate updating the same event due to new info in dialogue
    let dialogue_v2 = "User: 會議改期了，改到 2026-12-05 上午 10 點。\nAssistant: 沒問題，我來更新。";
    println!("  Extracting update from dialogue v2...");
    let updated_ids = engine.extract_reminders(conv_id, "Update meeting time.", dialogue_v2, timestamp, None).await?;
    ...
    sqlx::query("DELETE FROM reminders WHERE conversation_id = ?").bind(conv_id).execute(&pool).await?;
