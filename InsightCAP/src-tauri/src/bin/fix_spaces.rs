...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let _ = sqlx::query("ALTER TABLE spaces ADD COLUMN is_user_managed INTEGER DEFAULT 0").execute(&pool).await;
    
    println!("Refetching schema...");
    let row: (String,) = sqlx::query_as("SELECT sql FROM sqlite_master WHERE type='table' AND name='spaces'").fetch_one(&pool).await?;
