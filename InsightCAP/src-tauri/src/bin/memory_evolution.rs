...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (id, summary, expected) in &test_cases {
        ...
        let chunk_id = engine.process_conversation_summary(id, summary, None).await?;
    ...
    for r in rows {
        ...
        println!("  - ID: {} [{}] (Conf: {:.2}, Pending: {}) Content: {}...", k_name, k_type.to_uppercase(), confidence, pending, &content.chars().take(40).collect::<String>());
        println!("    Tags: {}", tags);
    }

    println!("\n>>> TEST COMPLETE. Cleaning up test data...");
    sqlx::query("DELETE FROM memory_chunks WHERE conversation_id LIKE 'conv-%'").execute(&pool).await?;
    sqlx::query("DELETE FROM conversations WHERE id LIKE 'conv-%'").execute(&pool).await?;
